...
(* ::Subsection::Closed:: *)
(*A package for studying combinatorial properties of posets.S*)
...
(**)
(*Updates by Curtis Greene, Erica Greene, 2008*)
(**)
(*Updates by Curtis Greene, Ian Burnette, 2010-11*)
(**)
...
(*         *:  New public command LongestChain[links,N] (previously called HeightFunction).*)
...
(*         *:  ZetaPoly[name,a,b,q] and ZetaPoly[name,q] added, replacing Z[name].  *)
(*         *:  OmegaGF[name,q] and OmegaBarGF[name,q] fixed.  (Degree of numerator wrong.)*)
(*         *:  IntervalP[name,bottom,top] added.*)
(*         *:  Sublattice[name,points] added.*)
(*         *: JoinSubLattice[name,points], MeetSubLattice[name,points] changed. Now return*)
...
(*         *:  LocateIndices[name,set] replaced by Locate[name,x] and LocateSet[name,set].*)
...
(*               *)*)
(**)
(*(* Changes 2.2d-> 2.9   (November 2007 Curtis Greene)*)
(*         *: Compatibility changes for Mathematica V6:*)
(*         *: Verbose -> BlahBlah*)
(*         *: Subsets[n] -> Sets[n]*)
(*         *: Up[name] -> Ups[name]*)
(*         *: Down[name] -> Downs[name]*)
(*         *: AspectRatio -> 1/GoldenRatio added to Diagram*)
(*               *) *)
(**)
(*(* Changes 2.9-> 2.97n   (June-July 2008/ Erica Greene, Curtis Greene)*)
(*         *: BlahBlah (controlling verbosity) eliminated; various other printed comments eliminated.*)
(*         *: Diagram command: major changes, including Manipulate option (with Save).*)
(*         *: Jiggled, Thinness options removed from Diagram (now in Manipulate control panel).*)
(*         *: Labels[name] added, with two sets of labels.*)
(*         *: ChangeLabel[name,fn,num], ChangeLabel[name,list,num], ResetLabel[name,num] added.*)
(*         *: Relabel[name] removed.*)
(*         *: SPM[n], L[n,theta] added (sandpile models).*)
(*         *: Sets[n], MajP[n], YoungsLattice[lambda] changed so that they generate*)
(*                 poset from bottom up.*)
(*         *: IsomorphicQ[name1,name2] changed so that it does a full isomorphism test.*)
(*         *: GetIsomorphism[name1,name2] added (returns isomorphism if one exists).*)
(*         *: IntervalQ[name,list] added.*)
(*         *: BuildSubPoset[name,set], BuildDualPoset[name] commands added. *)
(*         *: RestoreSubPosetLabels, RestoreDualPosetLabels removed (now automatic).*)
(*               *) *)
(**)
(*(* Changes 2.97n-> 2.98a   (August 2008/ Erica Greene, Curtis Greene)*)
(*         *: RWeakS[n] added.*)
(*         *: Div[n], NumP[n] changed so that they generate poset from bottom up.*)
(*         *: Ferrers[shape] added.*)
(*         *: AllTableaux[shape],STableaux[shape],STableaux[shape] added.*)
(*         *: TopSortings[name]: added.*)
(*         *: LinearExtensions[name]changed: now gives inverses of topological sortings.*)
(*         *: Fixed G[name,q] and Omega[name,q] so that they enumerate topological sortings.*)
(*              *) *)
(*(* Changes 2.99a-> 2.99b   (June 2010/ Ian Burnette, Curtis Greene)*)
(*      *: CleanRank (bool) option added to Diagram. (weakly ranked and unranked posets will diagram better with this option).*)
(*     *: NoOutput option added to RankedQ (turns off printing weak v. strong).*)
(*    *: Diagram[name, Manipulate->True] will now default to a Poset's saved settings if they exist.*)
(*   *: Added missing, and new items to the Zap command.*)
(*       *: Added StrongRankedQ[name] which will return True iff the poset is strongly ranked.*)
(**)*)
(*(* Changes 2.99b-> 2.99c   (July 2010/ Ian Burnette, Curtis Greene)*)
(*         *: Tabbed Diagram User interface added.*)
(*         *: InternalSettings[name], akin to SavedSettings[name], it standardizes the diagrams between tabs.*)
(*         *: Menu1 through Menu5 added, each Menu function acts as miniature of the way the old Diagram3 function worked.*)
(*			The difference being that they manipulate only a few specific properties.*)
(*         *: A 'Reset' button has been added to each tab, The old 'Reset' button was broken by the update to tabs. Reset now*)
(*			only resets the values manipuated on that specific tab.*)
(*         *)
(**)*)
(*(* Changes 2.99c-> 2.99d   (July 2010/ Ian Burnette, Curtis Greene)*)
(*         *: User Interface completely rebuilt using Control tools and Dynamic instead of the Manipulate command.*)
(*         *: InternalSettings[name] has been made much more internal, it now has a randomly generated tag to allow for multiple UI diagrams to be running at the same time.*)
(*         *: Menu1 through Menu5 functions incorporated into the Diagram3 function*)
(*         *: Added "Reset to saved" and "Reset to default" buttons.*)
(*         *: Fixed bug causing the zoom setting to not be saved properly.*)
(**)*)
(**)
(*(* Changes 2.99d-> 2.99e   (July 2010/ Ian Burnette, Curtis Greene)*)
(*         *: Added NewLabel and ChangeLabel commands in an attempt to make manipulating labels easier.*)
(*         *: More than two labels can now be stored at the same time.*)
(*         *: Command Change: Vee -> JoinL*)
(*         *: Command Change: Wedge -> MeetL*)
(*         *: The Label UI has been changed to allow the user to display any two of a Poset's labels.*)
(*         *: The ShowLabels -> True option of Diagram will now display the saved label settings.*)
(*         *: ShowLabels -> {a,b} will now display labels a and b. Likewise, ShowLabels -> i will show label i by itself.*)
(*         *: The Zoom setting now correctly saves and loads.*)
(*         *: DualOrderIdeals[name] command added.*)
(*         *: OrderIdeals[name] command added.*)
(*         *: RankedQ no longer prints to specify weakly/strongly ranked.*)
(**)*)
(* (* Changes 2.99e-> 3.00a   (Spring 2011/ Ian Burnette, Curtis Greene)*)
(*         *: Removed Jiggle option in favor of the ability to slide individual rows back and forth.*)
(*         *: Removed description text of some functions which no longer exist, and altered outdated/incorrect descriptions.*)
(*         *: The Diagram will automatically be 500x309 pixels when manipulate is used.*)
(*         *: Added "Reset to saved" and "Reset to default" buttons.*)
(*         *: Fixed bug causing the zoom setting to not be saved properly.*)
(**)*)
(* (* Changes 3.00a-> 3.00b   (June 2011/ Ian Burnette, Curtis Greene)*)
(*         *: Fixed RankQ bug caused by the introduction of sliding rows to the UI.*)
(*         *: Fixed bug causing the digram to be small when labels were loaded in manipulate.*)
(*         *: Renamed "Clean Rank" "Recalculate Rank" to be more clear.*)
(*         *: Added note that recalculating rank might result in needing to rerun the diagram command.*)
(*         *: changed "Reset to saved" buttons to "Load" buttons and added them on every tab.*)
(**)*)
(*(* Changes 3.00b-> 3.00c   (June 2012/Curtis Greene)*)
(*      *: Redefined as an actual package; checked and rewrote documentation.*)
(*         *: Fixed bug in StrongRankedQ.*)
(*      *: Eliminated InversePermutation (now standard in Mathematica 8)*)
(*         *: Tweaked positioning of labels. Added Background options.*)
(*    *: Fixed bug in PGraded (assumed indices, not elements, in some places). Added PGradedIndices*)
(*   *: Changed CleanRank to default to True.*)
(*  *: Added SpecialPoints[name,function] command.*)
(* *: Added StrictDowns[name],StrictUps[name].*)
(*       *: Added OrderIdeal[name,set], DualOrderIdeal[name,set], MaxElements[name,set], MinElements[name,set].*)
(*        *: Added Silent option (default=False) suppressing most printing.*)
(*         *: Minor bug fixes. Eliminated some unused lines in commands.*)
(**)*)
...
(*        *: Fix IntervalQ.*)
(*        *: IdealQ,PrincipalIdealQ,FilterQ,PrincipalFilterQ: add.*)
(*        *: Manipulate zoom controls: fine-tune them.*)
(* *)*)
...
of elements at rank k in P[name].";
...
less than or equal to P[name][[x]].";

StrictUps::usage = "StrictUps[name][[x]] is a list of the elements 
strictly greater than or equal to P[name][[x]].";

StrictDowns::usage = "StrictDowns[name][[x]] is a list of the 
elements strictly less than or equal to P[name][[x]].";
...
are isomorphic (by using the IsomorphicQ command)";
...
defines the subposet of P[name] induced by <points>. It 
...
of P[name]. This function will actually build the dual poset 
and relabel it with the original labels in P[name]. Input may 
...
SavedSettings[name]={};
  ...
  If[!Silent, Print["Building poset ",name,"  ..."]];
     ...
     Labels[name] = {P[name] , Range[Card[name]]};(*added this for double labels*)
    If[!Silent, Print["Done"]]
]
...
SortByRanks[name_] := Block[{newp},
	...
	CoverRelations[name] =
         Map[Position[newp,P[name][[#]]][[1,1]]&,CoverRelations[name],{2}];
	P[name] = newp;
...
Off[Unset::norep];
        If[S=={},(*no solutions*)
		PGraded[name]=.;PGradedIndices[name]=.;NK[name]=.;Rank[name]=.;H[name]=.;
	...
	Return[False],
        (*else solutions exist*) 
         ...
         minr = Min[solvars];If[minr<0,solvars=solvars+Table[-minr,{Card[name]}]];
	         PGraded[name]=.;PGradedIndices[name]=.;NK[name]=.;Rank[name]=.;RGF[name]=.;H[name]=.;
         Rank[name]=solvars;   
	    ];
 On[Unset::norep];
...
StrongRankedQ[name_] := StrongRankedQ[name] = Module[{templist={}, tranzeta= Transpose[ZetaP[name]]} ,
 If[RankedQ[name],For[i=1, i<= Length[tranzeta],i++,
If[Count[tranzeta[[i]], 1] ==1, AppendTo[templist, i]]];P[name][[templist]] ==PGraded[name][[1]], False]]
...
xy=Flatten[Table[Table[{(i+sliderowlist[[r]])/(nk[[r]]+1),r},{i,nk[[r]]}  ],{r,H[name]+1}],1];
...
transform=  Flatten[
	...
	Table[
		...
		(*if special pt, then...*) 
			{(((i-nk[[k]]/2-1/2)*stretchreg[[1]])+movereg[[1]]+specialpts[[1]]), (((k-H[name]/2-1)*stretchreg[[2]])+movereg[[2]]+specialpts[[2]])},    
		(*not special pt, else..*)
			{(((i-nk[[k]]/2-1/2)*stretchreg[[1]])+movereg[[1]]),(((k-H[name]/2-1)*stretchreg[[2]])+movereg[[2]])}],       
		{i,nk[[k]] }],
	{k, H[name]+1}
     ],1];
...
{0+zoom*(H[name]+2),H[name]+2-zoom*(H[name]+2)}},
     Frame->True,
     FrameTicks->None,
...
{mat,links,card,pointlist},If[name == automatic, name = Unique["SubPoset"]];
  ...
  If[Head[points]===List,pointlist=points,(*else*)
     pointlist=Position[Map[points,P[poset]],True]//Flatten];
  P[name]=Map[P[poset][[pointlist[[#]]]]&,P[name]];
Labels[name] ={P[name],Range[Card[name]]};
  ];
...
SpecialPoints[name_,function_] := Position[Map[function,P[name]],True]//Flatten;
